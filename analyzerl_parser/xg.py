...
def xg_model_source_columns() -> list[str]:
    ...
    return unique_list(model_pbp_columns())


def _ensure_model_source_columns(rows: pl.DataFrame) -> pl.DataFrame:
    additions = [
        pl.lit(None).alias(column)
        for column in model_pbp_columns()
        if column not in rows.columns
    ]
    return rows.with_columns(additions) if additions else rows
...
def _add_missing_feature_columns(
    ...
    for column in artifact['categorical_cols']:
        ...
        if column in shots.columns:
            additions.append(
                shots.get_column(column).cast(pl.Utf8, strict=False).alias(column)
            )
        else:
...
def _predict_xg(artifact: dict[str, Any], matrix) -> np.ndarray:
    model = artifact['model']
    if hasattr(model, 'predict_proba'):
        predictions = model.predict_proba(matrix)[:, 1]
    else:
        predictions = predict_scores(model, matrix)
...
def apply_xg_to_pbp(
    ...
    if frame.is_empty():
        if 'xG' in frame.columns:
            frame = frame.drop('xG')
        return frame.with_columns(pl.lit(None, dtype=pl.Float64).alias('xG'))

    frame = _ensure_model_source_columns(frame).with_row_index('_xg_row')
    shots = prepare_model_shots(frame.lazy()).collect()
    if shots.is_empty():
        return (
            frame.drop('_xg_row')
...
def apply_xg_to_file(
    ...
    if export_format == 'parquet':
        apply_xg_to_pbp(pl.read_parquet(path), xg_model_path).write_parquet(path)
        return
    if export_format == 'csv':
        ...
        rows = pl.read_csv(
            path,
            null_values=NULL_VALUES,
            schema_overrides=pbp_schema_overrides(set(header), model_pbp_columns()),
            infer_schema_length=10000,
        )
        apply_xg_to_pbp(rows, xg_model_path).write_csv(path)
        return
